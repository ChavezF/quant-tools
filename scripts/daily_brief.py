...
def build_brief(watchlist: list) -> str:
    ...
    try:
        ...
        # Earnings in next 7d
        if earnings:
            lines.append("  📅 Earnings 7d: " + ", ".join(f"{e['ticker']}({e['days']}d)" for e in earnings[:5]))
        else:
            lines.append("  📅 Earnings 7d: clear (no watchlist events)")
    except Exception as e:
    ...
    # Footer
    lines.append("")
    lines.append("📊 Run `screener` or `earnings-scan` for detail")
