...
def main():
    ...
    print(f"\n{'#'*100}")
    print(f"# EARNINGS BACKTEST v2 — 16Δ short strangle (and iron condor) entered 5d pre-earnings, held to 1d post")
    print(f"# Premium estimated from pre-earnings 30d RV (NO look-ahead on post-event move)")
    ...
    print(f"\n  Reading guide:")
    print(f"    Sharpe > 1.0 = risk-adjusted edge worth deploying capital")
    print(f"    Profit Factor > 1.5 = wins outsize losses reliably")
    print(f"    Worst single event > -3% = comfortable risk per trade")
    print(f"    Breach% > 25% = strikes too tight, widen or use defined risk")
    print(f"    OOS Win% within 10% of in-sample = strategy not overfit")
